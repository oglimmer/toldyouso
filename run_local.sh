...
toldyouso {Source:\"mvn\", Default-Type:\"local\", Version-Info: \"Tested with 3-jdk-11\"}
  ...
  -t toldyouso:docker:[TAG] #docker based build, default tag: latest, uses image https://hub.docker.com/_/maven
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="f3f85901e19b0c4bd51fce2773b745cf"
...
EOT3b28238d
			...
			export JAVA_OPTS="$JAVA_OPTS -DJAVA_OPTS=-Dtoldyouso.properties=/tmp/3b28238d/java.properties"
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			if [ "$TYPE_SOURCE_COUCHDB" == "docker" ]; then
				REPLVAR_TOLDYOUSO_COUCHDB="couchdb"
			elif [ "$TYPE_SOURCE_COUCHDB" == "local" ]; then
				REPLVAR_TOLDYOUSO_COUCHDB="host.docker.internal"
			fi
...
EOT3b28238d
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 -m 70M --net=toldyousonet --name=tomcat $ADD_HOST_INTERNAL -v "$(pwd)/localrun/3b28238d:/tmp/3b28238d" -e JAVA_OPTS="-Dtoldyouso.properties=/tmp/3b28238d/java.properties" -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps oglimmer/adoptopenjdk-tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
