...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "toldyousonet"; then
			verbosePrint "Starting docker network toldyousonet on 10.239.98.0/24"
			docker network create -d bridge --subnet 10.239.98.0/24 --gateway 10.239.98.1 "toldyousonet"
		else
			verbosePrint "Docker network toldyousonet already running"
		fi
	fi
}
...
Url: http://localhost:8080/toldyouso

Details for components:
toldyouso {Source:\"mvn\", Default-Type:\"local\", Version-Info: \"Tested with 3-jdk-11\"}
  -t toldyouso:local #build local and respect -j
  -t toldyouso:docker:[TAG] #docker based build, default tag: latest, uses image http://hub.docker.com/_/maven
couchdb {Source:\"couchdb\", Default-Type:\"docker:latest\", Version-Info: \"Tested with 1.7 & 2\"}
...
tomcat {Source:\"tomcat\", Default-Type:\"download:9\", Version-Info: \"Tested with 9\"}
...
if [ -z "$1" ]; then
	...
	ALL_COMPONENTS=(TOLDYOUSO COUCHDB TOMCAT)
	for comp in "$@"; do
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="b6b76d2969dbe7d256932c91d3f7e486"
...
verbosePrint "DEFAULT: TYPE_SOURCE_TOLDYOUSO=local"
TYPE_SOURCE_TOLDYOUSO=local
	...
	cat <<-EOF >Vagrantfile
		...
		Vagrant.configure("2") do |config|
		  ...
		  config.vm.provision "shell", inline: <<-SHELL
		    ...
		    echo "...then browse to http://localhost:8080/toldyouso"
		  SHELL
		end
	EOF
	vagrant up
...
# MvnPlugin // toldyouso
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
verbosePrint "MvnPlugin // toldyouso"

if [ "$START_TOLDYOUSO" = "YES" ]; then
	...
	for typeSourceElement in "${array[@]}"; do
		...
		if [ "$comp" == "toldyouso" ]; then
			TYPE_SOURCE_TOLDYOUSO=$type
			if [ "$TYPE_SOURCE_TOLDYOUSO" == "local" ]; then
				TYPE_SOURCE_TOLDYOUSO_PATH=$pathOrVersion
			else
				TYPE_SOURCE_TOLDYOUSO_VERSION=$pathOrVersion
			fi
		fi

	done

	if [ "$TYPE_SOURCE_TOLDYOUSO" == "docker" ]; then
		if [ -z "$TYPE_SOURCE_TOLDYOUSO_VERSION" ]; then
			TYPE_SOURCE_TOLDYOUSO_VERSION=latest
		fi

	fi

	verbosePrint "TYPE_SOURCE_TOLDYOUSO = $TYPE_SOURCE_TOLDYOUSO // TYPE_SOURCE_TOLDYOUSO_PATH = $TYPE_SOURCE_TOLDYOUSO_PATH // TYPE_SOURCE_TOLDYOUSO_VERSION = $TYPE_SOURCE_TOLDYOUSO_VERSION"
	...
	if [ "$TYPE_SOURCE_TOLDYOUSO" == "docker" ]; then
		...
		f_build() {
			verbosePrint "pwd=$(pwd)\ndocker run --name=toldyouso --rm -v $(pwd):/usr/src/build -v "$(pwd)/localrun/.m2":/root/.m2 -w /usr/src/build $dockerImage:$TYPE_SOURCE_TOLDYOUSO_VERSION mvn $MVN_CLEAN $MVN_OPTS package"
			if [ "$(jdk_version)" -lt 9 ]; then
				cp pom.xml pom.xml.bak
				sed '/<!-- needed since Java 9 - START -->/,/<!-- needed since Java 9 - END -->/d' pom.xml.bak >pom.xml
			fi
...
if [ "$START_COUCHDB" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_COUCHDB" == "docker" ]; then
		if [ ! -f ".couchdbPid" ]; then
			...
			verbosePrint "docker run --rm -d -p 5984:5984 --net=toldyousonet --name=couchdb $ADD_HOST_INTERNAL   couchdb:$TYPE_SOURCE_COUCHDB_VERSION"
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "download" ]; then
		if [ ! -f ".tomcatPid" ]; then
			...
			REPLVAR_TOLDYOUSO_COUCHDB="localhost"
...
couchdb.host=$REPLVAR_TOLDYOUSO_COUCHDB
...
EOT3b28238d
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			if [ "$TYPE_SOURCE_COUCHDB" == "docker" ]; then
				REPLVAR_TOMCAT_COUCHDB="couchdb"
			elif [ "$TYPE_SOURCE_COUCHDB" == "local" ]; then
				REPLVAR_TOMCAT_COUCHDB="host.docker.internal"
			fi
...
EOT3b28238d
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 --net=toldyousonet --name=tomcat $ADD_HOST_INTERNAL -v "$(pwd)/localrun/3b28238d:/tmp/3b28238d" -e JAVA_OPTS="-Dtoldyouso.properties=/tmp/3b28238d/java.properties"  -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
			dockerContainerIDtomcat=$(docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 \
				--net=toldyousonet --name=tomcat $ADD_HOST_INTERNAL \
...
if [ "$TAIL" == "YES" ]; then
	echo "http://localhost:8080/toldyouso"
